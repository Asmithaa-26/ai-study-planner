...
def ai_coach_message(goal, task, progress, pref):

    if progress < 0.3:
        tone = "You're just getting started — build strong foundations."
    elif progress < 0.7:
        tone = "You're improving! Consistency matters now."
    else:
        tone = "You're close to mastery — refine and practice deeply."

    style_tip = {
        "Reading": "Focus on understanding concepts clearly.",
        "Practice": "Code along and solve examples.",
        "Mixed": "Learn then immediately apply."
    }

    tip = style_tip.get(pref,"Keep learning steadily.")
...
Small focused sessions beat long distractions 🚀
"""
...
if today_tasks:
    coach_msg = ai_coach_message(
        g[1],
        today_tasks[0][3],
        progress,
        g[8] if len(g)>8 else "Mixed"
    )
    st.info(coach_msg)
